...
void RecordIOWriter::WriteRecord(const void* buf, size_t size) {
  ...
  for (uint32_t i = 0; i < lower_align; i += 4) {
    ...
    if (bhead[i] == magic[0] && bhead[i + 1] == magic[1] && bhead[i + 2] == magic[2] && bhead[i + 3] == magic[3]) {
...
RecordIOChunkReader::RecordIOChunkReader(InputSplit::Blob chunk, unsigned part_index, unsigned num_parts) {
