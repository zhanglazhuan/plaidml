...
class IndexedRecordIOSplitter : public InputSplitBase {
 public:
  IndexedRecordIOSplitter(FileSystem* fs, const char* uri, const char* index_uri, unsigned rank, unsigned nsplit,
                          const size_t batch_size, const bool shuffle, const int seed = 0) {
  ...
  bool IsTextParser(void) override { return false; }
  ...
  void SetBatchSize(int batch_size) { this->batch_size_ = batch_size; }
  bool NextChunkEx(Chunk* out_chunk) override { return NextBatchEx(out_chunk, batch_size_); }
