...
void TemporaryDirectory::RecursiveDelete(const std::string& path) {
  ...
  for (io::FileInfo info : file_list) {
    ...
    if (info.type == io::FileType::kDirectory) {
      ...
      if (std::remove(info.path.name.c_str()) != 0) {
        LOG(INFO) << "Couldn't remove file " << info.path.name << "; you may want to remove it manually";
      }
    }
  }
#if _WIN32
  ...
  if (rmdir_success) {
    ...
    LOG(INFO) << "~TemporaryDirectory(): "
              << "Could not remove temporary directory " << path << "; you may want to remove it manually";
  }
}
}  // namespace dmlc
