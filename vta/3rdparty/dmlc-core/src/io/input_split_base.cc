...
namespace dmlc {
namespace io {
void InputSplitBase::Init(FileSystem* filesys, const char* uri, size_t align_bytes, const bool recurse_directories) {
...
void InputSplitBase::ResetPartition(unsigned rank, unsigned nsplit) {
  ...
  file_ptr_ = std::upper_bound(file_offset_.begin(), file_offset_.end(), offset_begin_) - file_offset_.begin() - 1;
  file_ptr_end_ = std::upper_bound(file_offset_.begin(), file_offset_.end(), offset_end_) - file_offset_.begin() - 1;
  if (fs_ != NULL) {
    delete fs_;
    fs_ = NULL;
  }
...
void InputSplitBase::BeforeFirst(void) {
  if (offset_begin_ >= offset_end_) return;
  size_t fp = std::upper_bound(file_offset_.begin(), file_offset_.end(), offset_begin_) - file_offset_.begin() - 1;
  if (file_ptr_ != fp) {
    delete fs_;
    file_ptr_ = fp;
    fs_ = filesys_->OpenForRead(files_[file_ptr_].path);
  ...
  while (true) {
    ...
    if (n == 0) {
      ...
      if (offset_curr_ != file_offset_[file_ptr_ + 1]) {
        LOG(ERROR) << "curr=" << offset_curr_ << ",begin=" << offset_begin_ << ",end=" << offset_end_
                   << ",fileptr=" << file_ptr_ << ",fileoffset=" << file_offset_[file_ptr_ + 1];
...
bool InputSplitBase::Chunk::Append(InputSplitBase* split, size_t buffer_size) {
  ...
  while (true) {
    ...
    // set back to 0 for string safety
    data.back() = 0;
    if (!split->ReadChunk(reinterpret_cast<char*>(BeginPtr(data)) + previous_size, &size)) return false;
    if (size == 0) {
      data.resize(data.size() * 2);
    } else {
      begin = reinterpret_cast<char*>(BeginPtr(data));
