...
bool IndexedRecordIOSplitter::ExtractNextRecord(Blob* out_rec, Chunk* chunk) {
  ...
  while (cflag != 3U) {
    ...
    if (clen != 0) {
      std::memmove(reinterpret_cast<char*>(out_rec->dptr) + out_rec->size, chunk->begin + 2 * sizeof(uint32_t), clen);
      out_rec->size += clen;
    }
    chunk->begin += 2 * sizeof(uint32_t) + (((clen + 3U) >> 2U) << 2U);
  }
...
bool IndexedRecordIOSplitter::NextChunk(Blob* out_chunk) { return this->NextBatch(out_chunk, batch_size_); }
  ...
  if (shuffle_) {
    ...
    while (n_read < n) {
      if (current_index_ < permutation_.size()) {
        ...
        size_t new_file_ptr =
            std::upper_bound(file_offset_.begin(), file_offset_.end(), offset_curr_) - file_offset_.begin() - 1;
