...
class SingleThreadedInputSplit : public InputSplit {
 public:
  explicit SingleThreadedInputSplit(InputSplitBase* base, const size_t batch_size)
      : buffer_size_(InputSplitBase::kBufferSize), batch_size_(batch_size), base_(base), tmp_chunk_(NULL) {}
  ...
  virtual bool NextRecord(Blob* out_rec) {
    if (tmp_chunk_ == NULL) {
      if (!NextProducer(&tmp_chunk_)) return false;
    }
