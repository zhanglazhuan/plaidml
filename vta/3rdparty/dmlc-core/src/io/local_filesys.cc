...
FileInfo LocalFileSystem::GetPathInfo(const URI& path) {
  ...
  if (stat(path.name.c_str(), &sb) == -1) {
    ...
    if (lstat(path.name.c_str(), &sb) == 0) {
      ret.size = 0;
      ret.type = kFile;
      LOG(INFO) << "LocalFileSystem.GetPathInfo: detected symlink " << path.name << " error: " << strerror(errsv);
      return ret;
    }
#endif  // _WIN32
...
SeekStream* LocalFileSystem::OpenForRead(const URI& path, bool allow_null) { return Open(path, "r", allow_null); }
}  // namespace io
}  // namespace dmlc
