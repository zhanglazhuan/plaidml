...
class HDFSStream : public SeekStream {
 public:
  HDFSStream(hdfsFS fs, int* ref_counter, hdfsFile fp) : fs_(fs), ref_counter_(ref_counter), fp_(fp) {}
...
FileInfo HDFSFileSystem::GetPathInfo(const URI& path) {
  CHECK(path.protocol == "hdfs://" || path.protocol == "viewfs://") << "HDFSFileSystem only works with hdfs and viewfs";
...
SeekStream* HDFSFileSystem::OpenForRead(const URI& path, bool allow_null) { return Open(path, "r", allow_null); }
}  // namespace io
}  // namespace dmlc
