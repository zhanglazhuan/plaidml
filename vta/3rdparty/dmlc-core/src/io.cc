...
FileSystem* FileSystem::GetInstance(const URI& path) {
  ...
  if (path.protocol == "hdfs://" || path.protocol == "viewfs://") {
    ...
    if (path.host.length() == 0) {
      ...
      if (path.host.length() != 0) {
        CHECK("viewfs://" + path.host == defaultFS) << "viewfs is only supported as a fs.defaultFS.";
      }
      return HDFSFileSystem::GetInstance("default");
    } else {
...
InputSplit* InputSplit::Create(const char* uri_, const char* index_uri_, unsigned part, unsigned nsplit,
                               const char* type, const bool shuffle, const int seed, const size_t batch_size,
  ...
  if (!strcmp(type, "text")) {
    ...
    if (index_uri_ != nullptr) {
      ...
      split = new IndexedRecordIOSplitter(FileSystem::GetInstance(path), spec.uri.c_str(), index_spec.uri.c_str(), part,
                                          nsplit, batch_size, shuffle, seed);
    } else {
      LOG(FATAL) << "need to pass index file to use IndexedRecordIO";
    }
  } else if (!strcmp(type, "recordio")) {
    split = new RecordIOSplitter(FileSystem::GetInstance(path), spec.uri.c_str(), part, nsplit, recurse_directories);
  } else {
