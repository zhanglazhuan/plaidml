...
class LibFMParser : public TextParserBase<IndexType, DType> {
 public:
  ...
  explicit LibFMParser(InputSplit* source, const std::map<std::string, std::string>& args, int nthread)
...
void LibFMParser<IndexType, DType>::ParseBlock(const char* begin, const char* end,
  ...
  if (param_.indexing_mode > 0 ||
      (param_.indexing_mode < 0 && !out->index.empty() && min_feat_id > 0 && !out->field.empty() && min_field_id > 0)) {
