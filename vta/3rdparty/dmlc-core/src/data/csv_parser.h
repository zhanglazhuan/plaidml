...
struct CSVParserParam : public Parameter<CSVParserParam> {
  ...
  DMLC_DECLARE_PARAMETER(CSVParserParam) {
    ...
    DMLC_DECLARE_FIELD(label_column).set_default(-1).describe("Column index (0-based) that will put into label.");
    DMLC_DECLARE_FIELD(delimiter).set_default(",").describe("Delimiter used in the csv file.");
    DMLC_DECLARE_FIELD(weight_column).set_default(-1).describe("Column index that will put into instance weights.");
  }
};
...
class CSVParser : public TextParserBase<IndexType, DType> {
 public:
  explicit CSVParser(InputSplit* source, const std::map<std::string, std::string>& args, int nthread)
...
void CSVParser<IndexType, DType>::ParseBlock(const char* begin, const char* end,
    ...
    while (p != lend) {
      ...
      if (p == lend && idx == 0) {
        LOG(FATAL) << "Delimiter \'" << param_.delimiter << "\' is not found in the line. "
                   << "Expected \'" << param_.delimiter << "\' as the delimiter to separate fields.";
      }
      if (p != lend) ++p;
    }
    // skip empty line
