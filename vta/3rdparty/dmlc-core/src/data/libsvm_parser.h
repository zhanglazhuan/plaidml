...
class LibSVMParser : public TextParserBase<IndexType> {
 public:
  ...
  explicit LibSVMParser(InputSplit* source, const std::map<std::string, std::string>& args, int nthread)
...
void LibSVMParser<IndexType, DType>::ParseBlock(const char* begin, const char* end,
  ...
  if (param_.indexing_mode > 0 || (param_.indexing_mode < 0 && !out->index.empty() && min_feat_id > 0)) {
