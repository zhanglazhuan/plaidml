...
struct RowBlockContainer {
  ...
  IndexType max_index;
  // constructor
  RowBlockContainer(void) { this->Clear(); }
  ...
  inline size_t Size(void) const { return offset.size() - 1; }
  /*! \return estimation of memory cost of this container */
  inline size_t MemCostBytes(void) const {
    return offset.size() * sizeof(size_t) + label.size() * sizeof(real_t) + weight.size() * sizeof(real_t) +
           qid.size() * sizeof(size_t) + field.size() * sizeof(IndexType) + index.size() * sizeof(IndexType) +
  ...
  inline void Push(Row<I, DType> row) {
    ...
    if (row.field != NULL) {
      for (size_t i = 0; i < row.length; ++i) {
        CHECK_LE(row.field[i], std::numeric_limits<IndexType>::max()) << "field exceed numeric bound of current type";
    ...
    for (size_t i = 0; i < row.length; ++i) {
      CHECK_LE(row.index[i], std::numeric_limits<IndexType>::max()) << "index exceed numeric bound of current type";
  ...
  inline void Push(RowBlock<I, DType> batch) {
    ...
    for (size_t i = 0; i < ndata; ++i) {
      CHECK_LE(batch.index[i], std::numeric_limits<IndexType>::max()) << "index  exceed numeric bound of current type";
