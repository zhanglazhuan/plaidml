...
class BasicRowIter : public RowBlockIter<IndexType, DType> {
 public:
  ...
  virtual void BeforeFirst(void) { at_head_ = true; }
  ...
  virtual const RowBlock<IndexType, DType>& Value(void) const { return row_; }
  virtual size_t NumCol(void) const { return static_cast<size_t>(data_.max_index) + 1; }
