...
class Tokenizer {
 ...
 private:
  int PeekChar() { return is_.peek(); }
  void EatChar() { is_.get(); }
...
//////////////////////// Config /////////////////////////////
Config::Config(bool m) : multi_value_(m) { Clear(); }
...
void Config::LoadFromStream(istream& is) {
  ...
  try {
    while (true) {
      ...
      if (eqop.buf != "=") {
        LOG(ERROR) << "Parsing error: expect format \"k = v\"; but got \"" << key.buf << eqop.buf << value.buf << "\"";
      }
...
Config::ConfigIterator Config::begin() const { return ConfigIterator(0, this); }

Config::ConfigIterator Config::end() const { return ConfigIterator(order_.size(), this); }
...
Config::ConfigIterator::ConfigIterator(size_t i, const Config* c) : index_(i), config_(c) { FindNextIndex(); }
...
bool Config::ConfigIterator::operator!=(const Config::ConfigIterator& rhs) const { return !(operator==(rhs)); }
