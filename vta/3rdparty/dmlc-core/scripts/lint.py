...
class LintHelper(object):
    ...
    def __init__(self):
        ...
        # setup pylint
        self.pylint_opts = [
            '--extension-pkg-whitelist=numpy', '--disable=' + ','.join(pylint_disable)
        ]
        ...
        cpplint._SetFilters(','.join([
            '-build/c++11', '-build/namespaces', '-build/include,', '+build/include_what_you_use',
            '+build/include_order'
        ]))
    ...
    def process_python(self, path):
        ...
        (pylint_stdout, pylint_stderr) = epylint.py_run(' '.join([str(path)] + self.pylint_opts),
                                                        return_std=True)
        emap = {}
