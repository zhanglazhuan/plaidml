...
try:
    ...
    def _run(prog, env, resources):
        cwd = os.getcwd()
        pymesos.subprocess.check_call(prog,
                                      shell=True,
                                      env=env,
                                      cwd=cwd,
                                      cpus=resources['cpus'],
    ...
    def _run(prog, env, resources):
        ...
        cmd = ('mesos-execute --master=%s --name=\'%s\''
...
def submit(args):
    ...
    if not _USE_PYMESOS:
        logging.warning('No PyMesos found, use mesos-execute instead,' ' no task output available')
