...
TEST(Parameter, parsing_float) {
  ...
  for (const char* s : {"inf", "+inf", "-inf", "INF", "+INF", "-INF", "infinity", "+infinity", "-infinity", "INFINITY",
                        "+INFINITY", "-INFINITY"}) {
  ...
  for (const char* s : {"nan", "NAN", "nan(foobar)", "NAN(FooBar)", "NaN", "NaN(foo_bar_12)", "+nan", "+NAN",
                        "+nan(foobar)", "+NAN(FooBar)", "+NaN", "+NaN(foo_bar_12)", "-nan", "-NAN", "-nan(foobar)",
                        "-NAN(FooBar)", "-NaN", "-NaN(foo_bar_12)"}) {
    kwargs["float_param"] = s;
    ASSERT_NO_THROW(param.Init(kwargs));
