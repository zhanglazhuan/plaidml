...
static inline int dmlc_usleep(useconds_t useconds) { return usleep(useconds); }
...
TEST(ThreadGroup, ThreadLaunchQueueThread) {
  ...
  std::shared_ptr<BQ> queue_thread = std::make_shared<BQ>("BlockingQueueThread", thread_group.get());
...
static inline uint64_t GetDurationInNanoseconds(const Tick& t1, const Tick& t2) {
  return static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count());
}
static inline uint64_t GetDurationInNanoseconds(const Tick& since) { return GetDurationInNanoseconds(since, Now()); }
...
TEST(ThreadGroup, TimerThread) {
  ...
  size_t count = 0;
  // Launch the queue thread, passing queue item handler as lambda
  dmlc::TimerThread<Duration>::start(timer_thread, Duration(TIMER_PERIOD), [timer_thread, start_time, &count]() -> int {
    if ((count + 1) % 5 == 0) {
      // output slows it down a bit, so print fewer times
      std::cout << "[" << (count + 1) << "] TIME: " << GetDurationInMilliseconds(start_time) << std::endl << std::flush;
    }
    ++count;
    return 0;  // return 0 means continue
  });
...
TEST(ThreadGroup, TimerThreadSimple) {
  ...
  size_t count = 0;
  // Launch the queue thread, passing queue item handler as lambda
  dmlc::CreateTimer("TimerThreadSimple", Duration(TIMER_PERIOD), thread_group.get(), [start_time, &count]() -> int {
    if ((count + 1) % 5 == 0) {
      // output slows it down a bit, so print fewer times
      std::cout << "[" << (count + 1) << "] TIME: " << GetDurationInMilliseconds(start_time) << std::endl << std::flush;
    }
