...
class MyClass {
 public:
  ...
  inline bool operator==(const MyClass& other) const { return data_ == other.data_; }
...
TEST(Serializer, endian_detection) {
  std::string little_endian_flag = dmlc::GetEnv("DMLC_UNIT_TEST_LITTLE_ENDIAN", std::string("NULL"));
  if (little_endian_flag == "0" || little_endian_flag == "1") {
    ...
    std::cout << "\x1B[33mWarning\u001B[0m: Skipping this test because DMLC_UNIT_TEST_LITTLE_ENDIAN is not set"
              << std::endl;
  }
}
