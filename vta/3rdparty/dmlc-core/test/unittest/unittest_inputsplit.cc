...
inline void CountDimensions(dmlc::Parser<uint32_t>* parser, size_t* out_num_row, size_t* out_num_col) {
  size_t num_row = 0;
  size_t num_col = 0;
  while (parser->Next()) {
...
TEST(InputSplit, test_split_csv_noeol) {
  size_t num_row, num_col;
  {
    ...
    {
      std::unique_ptr<dmlc::Parser<uint32_t>> parser(dmlc::Parser<uint32_t>::Create(tempdir.path.c_str(), 0, 1, "csv"));
...
TEST(InputSplit, test_split_libsvm) {
  size_t num_row, num_col;
  {
    ...
    for (int file_id = 0; file_id < nfile; ++file_id) {
      std::ofstream of(tempdir.path + "/test_" + std::to_string(file_id) + ".libsvm", std::ios::binary);
