...
int main(int argc, char* argv[]) {
  ...
  RowBlockIter<index_t>* iter = RowBlockIter<index_t>::Create(argv[1], atoi(argv[2]), atoi(argv[3]), format);
  ...
  while (iter->Next()) {
    ...
    LOG(INFO) << (bytes_read >> 20UL) << " MB read " << ((bytes_read >> 20UL) / tdiff) << " MB/sec";
  }
  return 0;
}
