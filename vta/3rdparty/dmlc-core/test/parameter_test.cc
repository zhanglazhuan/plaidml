...
struct Param : public dmlc::Parameter<Param> {
  ...
  DMLC_DECLARE_PARAMETER(Param) {
    DMLC_DECLARE_FIELD(num_hidden).set_range(0, 1000).describe("Number of hidden unit in the fully connected layer.");
    DMLC_DECLARE_FIELD(learning_rate).set_default(0.01f).describe("Learning rate of SGD optimization.");
    DMLC_DECLARE_FIELD(act).add_enum("relu", 1).add_enum("sigmoid", 2).describe("Activation function type.");
    DMLC_DECLARE_FIELD(name).set_default("A").describe("Name of the net.");
  }
};
...
struct SecondParam : public dmlc::Parameter<SecondParam> {
  ...
  DMLC_DECLARE_PARAMETER(SecondParam) {
    DMLC_DECLARE_FIELD(num_data).set_range(0, 1000).describe("Number of data points");
  }
};
