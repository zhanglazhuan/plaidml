...
class ThreadedIter : public DataIter<DType> {
 public:
  ...
  class Producer {
   public:
    ...
    virtual void BeforeFirst(void) { NotImplemented(); }
    /*!
   ...
   */
  inline void Init(std::function<bool(DType**)> next, std::function<void()> beforefirst = NotImplemented);
  /*!
  ...
  virtual void BeforeFirst(void) {
    ...
    consumer_cond_.wait(lock, [this]() { return producer_sig_processed_; });
 ...
 private:
  ...
  inline static void NotImplemented(void) { LOG(FATAL) << "BeforeFirst is not supported"; }
...
template <typename DType>
inline void ThreadedIter<DType>::Init(Producer* producer, bool pass_ownership) {
...
template <typename DType>
inline void ThreadedIter<DType>::Init(std::function<bool(DType**)> next, std::function<void()> beforefirst) {
      ...
      try {
        ...
        bool next_notify = false;
        {
          ...
          if (producer_sig_ == kBeforeFirst) {
            ...
            lock.unlock();
            if (next_notify) consumer_cond_.notify_all();
          }
        }
        return;
      }
...
inline bool ThreadedIter<DType>::Next(DType** out_dptr) {
  ...
  ++nwait_consumer_;
  consumer_cond_.wait(lock, [this]() { return queue_.size() != 0 || produce_end_.load(std::memory_order_acquire); });
  --nwait_consumer_;
  if (queue_.size() != 0) {
