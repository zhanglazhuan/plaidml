 ...
 public:
  ...
  class Thread {
   ...
   private:
     ...
     */
    template <typename StartFunction, typename... Args>
    static int entry_and_exit_f(std::shared_ptr<Thread> pThis, StartFunction start_function, Args... args);
    /*! \brief Thread name */
    std::string name_;
  ...
  inline bool create(const std::string& threadName, bool auto_remove, StartFunction start_function, Args... args) {
 ...
 public:
  ...
  static bool launch_run(std::shared_ptr<BQT> pThis, SecondaryFunction secondary_function) {
    return ThreadGroup::Thread::launch(
        pThis, true,
        [](std::shared_ptr<BQT> pThis, SecondaryFunction secondary_function) { return pThis->run(secondary_function); },
...
class TimerThread : public ThreadGroup::Thread {
 ...
 public:
   ...
   * \brief Destructor
   */
  ~TimerThread() override { request_shutdown(); }
 ...
 */
template <typename StartFunction, typename... Args>
inline int ThreadGroup::Thread::entry_and_exit_f(std::shared_ptr<Thread> pThis, StartFunction start_function,
                                                 Args... args) {
  int rc;
  if (pThis) {
...
template <typename StartFunction, typename... Args>
inline bool ThreadGroup::Thread::launch(std::shared_ptr<Thread> pThis, bool autoRemove, StartFunction start_function,
                                        Args... args) {
  ...
  pThis->thread_ =
      new std::thread(Thread::template entry_and_exit_f<StartFunction, Args...>, pThis, start_function, args...);
