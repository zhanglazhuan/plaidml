...
inline std::string Demangle(char const* msg_str) {
  ...
  if (((symbol_start = msg.find("_Z")) != string::npos) && (symbol_end = msg.find_first_of(" +", symbol_start))) {
...
inline std::string StackTrace(size_t start_frame = 1, const size_t stack_size = DMLC_LOG_STACK_TRACE_SIZE) {
  using std::string;
...
class LogCheckError {
 public:
  ...
  LogCheckError(LogCheckError&& other) : str(other.str) { other.str = nullptr; }
  ~LogCheckError() {
...
#define DEFINE_CHECK_FUNC(name, op)                                                                              \
  ...
  inline LogCheckError LogCheck##name(int x, int y) { return LogCheck##name<int, int>(x, y); }
#else
#define DEFINE_CHECK_FUNC(name, op)                             \
  template <typename X, typename Y>                             \
  inline LogCheckError LogCheck##name(const X& x, const Y& y) { \
...
#define CHECK_NOTNULL(x)                                                                             \
  ((x) == NULL ? dmlc::LogMessageFatal(__FILE__, __LINE__).stream() << "Check  notnull: " #x << ' ', \
   (x) : (x))  // NOLINT(*)
...
#define DLOG_IF(severity, condition) (true || !(condition)) ? (void)0 : dmlc::LogMessageVoidify() & LOG(severity)
#endif
