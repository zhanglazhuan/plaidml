...
inline FloatType ParseFloat(const char* nptr, char** endptr) {
  ...
  constexpr FloatType kMaxSignificandForMaxExponent =
      static_cast<FloatType>(std::is_same<FloatType, double>::value ? 1.79769313486231570 : 3.402823466);
  ...
  constexpr FloatType kMaxSignificandForNegMaxExponent =
      static_cast<FloatType>(std::is_same<FloatType, double>::value ? 2.22507385850720139 : 1.175494351);
  // If a floating-point value has -kMaxExponent, what is
  //   the largest possible significand value?
#else
  const unsigned kMaxExponent = (sizeof(FloatType) == sizeof(double) ? 308U : 38U);
  const FloatType kMaxSignificandForMaxExponent =
      static_cast<FloatType>(sizeof(FloatType) == sizeof(double) ? 1.79769313486231570 : 3.402823466);
  const FloatType kMaxSignificandForNegMaxExponent =
      static_cast<FloatType>(sizeof(FloatType) == sizeof(double) ? 2.22507385850720139 : 1.175494351);
#endif
  ...
  // Handle INF and NAN
  {
    ...
    if (i == 3 || i == 8) {
      ...
      return sign ? std::numeric_limits<FloatType>::infinity() : -std::numeric_limits<FloatType>::infinity();
    } else {
      p -= i;
    }
    ...
    if (i == 3) {
      ...
      static_assert(std::numeric_limits<FloatType>::has_quiet_NaN, "Only system with quiet NaN is supported");
      if (endptr) *endptr = (char*)p;  // NOLINT(*)
  ...
  if ((*p == 'e') || (*p == 'E')) {
    ...
    if (expon == kMaxExponent &&
        ((!frac && value > kMaxSignificandForMaxExponent) || (frac && value < kMaxSignificandForNegMaxExponent))) {
      if (CheckRange) {
        errno = ERANGE;
        if (endptr) *endptr = (char*)p;  // NOLINT(*)
        return std::numeric_limits<FloatType>::infinity();
 ...
 * \return Converted floating-point value, in float type
 */
inline float strtof_check_range(const char* nptr, char** endptr) { return ParseFloat<float, true>(nptr, endptr); }
 ...
 * \return Converted floating-point value, in float type
 */
inline double strtod_check_range(const char* nptr, char** endptr) { return ParseFloat<double, true>(nptr, endptr); }
...
#ifdef DMLC_USE_CXX11
  static_assert(std::is_unsigned<UnsignedIntType>::value && std::is_integral<UnsignedIntType>::value,
...
class Str2T<int32_t> {
 public:
   ...
   */
  static inline int32_t get(const char* begin, const char* end) { return ParseSignedInt<int32_t>(begin, NULL, 10); }
};
...
class Str2T<uint32_t> {
 public:
   ...
   */
  static inline uint32_t get(const char* begin, const char* end) { return ParseUnsignedInt<uint32_t>(begin, NULL, 10); }
};
...
class Str2T<int64_t> {
 public:
   ...
   */
  static inline int64_t get(const char* begin, const char* end) { return ParseSignedInt<int64_t>(begin, NULL, 10); }
};
...
class Str2T<uint64_t> {
 public:
   ...
   * \return Converted value, as unsigned 64-bit integer
   */
  static inline uint64_t get(const char* begin, const char* end) { return ParseUnsignedInt<uint64_t>(begin, NULL, 10); }
};
...
class Str2T<float> {
 public:
   ...
   * \return Converted value, in float type
   */
  static inline float get(const char* begin, const char* end) { return atof(begin); }
};
...
class Str2T<double> {
 public:
   ...
   * \return Converted value, in double type
   */
  static inline double get(const char* begin, const char* end) { return strtod(begin, 0); }
};
...
inline int ParsePair(const char* begin, const char* end, const char** endptr, T1& v1, T2& v2) {  // NOLINT(*)
  const char* p = begin;
...
inline int ParseTriple(const char* begin, const char* end, const char** endptr, T1& v1, T2& v2, T3& v3) {  // NOLINT(*)
  const char* p = begin;
  while (p != end && !isdigitchars(*p)) ++p;
  if (p == end) {
    *endptr = end;
