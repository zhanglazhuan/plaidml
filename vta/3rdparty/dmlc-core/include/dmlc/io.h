...
class ostream : public std::basic_ostream<char> {
 public:
   ...
   */
  explicit ostream(Stream* stream, size_t buffer_size = (1 << 10)) : std::basic_ostream<char>(NULL), buf_(buffer_size) {
...
class istream : public std::basic_istream<char> {
 public:
   ...
   */
  explicit istream(Stream* stream, size_t buffer_size = (1 << 10)) : std::basic_istream<char>(NULL), buf_(buffer_size) {
    this->set_stream(stream);
  }
