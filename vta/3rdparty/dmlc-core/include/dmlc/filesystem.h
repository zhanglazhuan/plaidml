...
class TemporaryDirectory {
 public:
  ...
  explicit TemporaryDirectory(bool verbose = false) : verbose_(verbose) {
    ...
    std::generate(uniqstr.begin(), uniqstr.end(), [&dis, &gen, &letters]() -> char { return letters[dis(gen)]; });
  ...
  ~TemporaryDirectory() { RecursiveDelete(path); }
 ...
 private:
  ...
  inline bool IsSymlink(const std::string& path) {
    ...
    CHECK_NE(attr, INVALID_FILE_ATTRIBUTES) << "dmlc::TemporaryDirectory::IsSymlink(): Unable to read file attributes";
    return attr & FILE_ATTRIBUTE_REPARSE_POINT;
#else   // _WIN32
    struct stat sb;
    CHECK_EQ(lstat(path.c_str(), &sb), 0) << "dmlc::TemporaryDirectory::IsSymlink(): Unable to read file attributes";
