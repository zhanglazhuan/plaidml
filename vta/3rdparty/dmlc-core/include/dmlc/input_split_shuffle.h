...
class InputSplitShuffle : public InputSplit {
 public:
  ...
  virtual void HintChunkSize(size_t chunk_size) { source_->HintChunkSize(chunk_size); }
  virtual size_t GetTotalSize(void) { return source_->GetTotalSize(); }
  ...
  InputSplitShuffle(const char* uri, unsigned part_index, unsigned num_parts, const char* type,
      ...
      : part_index_(part_index), num_parts_(num_parts), num_shuffle_parts_(num_shuffle_parts), cur_shuffle_idx_(0) {
