...
class ConcurrentBlockingQueue {
 ...
 private:
  struct Entry {
    T data;
    int priority;
    inline bool operator<(const Entry& b) const { return priority < b.priority; }
  };
...
inline void Spinlock::unlock() noexcept(true) { lock_.clear(std::memory_order_release); }

template <typename T, ConcurrentQueueType type>
ConcurrentBlockingQueue<T, type>::ConcurrentBlockingQueue() : exit_now_{false}, nwait_consumer_{0} {}
