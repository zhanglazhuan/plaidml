...
class LuaState {
 public:
   ...
   */
  inline LuaRef Eval(const std::string& lua_code) { return this->Eval(lua_code.c_str()); }
  /*!
   ...
   */
  inline bool SameLuaState(lua_State* L) const { return L_ == L; }
...
struct MapHandler {
  ...
  static inline ContainerType Get(lua_State* L, int index, LuaState* s) {
    ContainerType ret;
    CHECK(lua_istable(L, index)) << "Expected a table but get " << lua_typename(L, lua_type(L, index)) << '\'';
...
template <typename T>
struct Handler : public std::conditional<std::is_arithmetic<T>::value, NumberHandler<T>, UndefinedHandler>::type {};
...
struct Handler<std::string> {
  ...
  static inline void Push(lua_State* L, const std::string& v) { lua_pushstring(L, v.c_str()); }
};
...
struct Handler<std::nullptr_t> {
  ...
  static inline void Push(lua_State* L, const std::nullptr_t& v) { lua_pushnil(L); }
};
...
inline void LuaState::PRun_(F f) {
  if (option_ != kLocking) {
    ...
    if (option_ == kThreadLocal) {
      CHECK_EQ(ThreadLocalState(), this) << "Invoke lua from a different thread in ThreadLocal mode.";
    }
    f(L_);
...
inline LuaState* LuaState::ThreadLocalState() { return ThreadLocalStore<LuaState>::Get(); }
...
inline bool LuaRef::is_nil() const { return state_ == nullptr; }
...
inline LuaRef LuaRef::operator()(Args&&... args) const {
  ...
  state_->PRun_([this, nargs, &targ, &ret](lua_State* L) {
    ...
    CHECK(lua_isfunction(L, -1)) << "Expect to invoke a function but type='" << lua_typename(L, lua_type(L, -1))
                                 << '\'';
