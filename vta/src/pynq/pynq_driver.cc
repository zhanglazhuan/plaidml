...
vta_phy_addr_t VTAMemGetPhyAddr(void* buf) { return cma_get_phy_addr(buf); }
...
void* VTAMapRegister(uint32_t addr) {
  ...
  return mmap(NULL, (VTA_IP_REG_MAP_RANGE + virt_offset), PROT_READ | PROT_WRITE, MAP_SHARED, mmap_file, virt_base);
}
...
void VTADeviceFree(VTADeviceHandle handle) { delete static_cast<VTADevice*>(handle); }
