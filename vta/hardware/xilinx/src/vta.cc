...
void reset_mem(memop_sram_T& sram_idx, memop_sram_T range, DATA_T mem[][MAT_AXI_RATIO]) {
...
void load_pad_2d(volatile DATA_T* src, DATA_T dst[][MAT_AXI_RATIO], memop_sram_T sram_idx, memop_dram_T dram_idx,
                 memop_size_T y_size, memop_size_T x_size, memop_stride_T x_stride, memop_pad_T x_pad_0,
                 memop_pad_T x_pad_1, memop_sram_T y_offset_0, memop_sram_T y_offset_1) {
#pragma HLS INLINE
...
#pragma HLS PIPELINE
    ...
    memcpy(&dst[sram_idx][0], (const DATA_T*)&src[dram_idx * MAT_AXI_RATIO], x_size * ELEM_BYTES);
    sram_idx += x_size;
    dram_idx += x_stride;
...
template <typename DATA_T, int MAT_AXI_RATIO, int ELEM_BYTES>
void load_2d(volatile DATA_T* src, DATA_T dst[][MAT_AXI_RATIO], memop_sram_T sram_idx, memop_dram_T dram_idx,
             memop_size_T y_size, memop_size_T x_size, memop_stride_T x_stride) {
#pragma HLS INLINE
...
void read_tensor(IDX_T idx, WIDE_T src[][NARROW_W * Y_DIM * X_DIM / WIDE_W], NARROW_T dst[Y_DIM][X_DIM]) {
#pragma HLS INLINE
...
template <typename WIDE_T, typename NARROW_T, typename IDX_T, int WIDE_W, int NARROW_W, int Y_DIM, int X_DIM>
void write_tensor(IDX_T idx, NARROW_T src[Y_DIM][X_DIM], WIDE_T dst[][NARROW_W * Y_DIM * X_DIM / WIDE_W]) {
#pragma HLS INLINE
...
void fetch(uint32_t insn_count, volatile insn_T* insns, hls::stream<insn_T>& load_queue,
           hls::stream<insn_T>& gemm_queue, hls::stream<insn_T>& store_queue) {
...
void load(volatile bus_T* inputs, volatile bus_T* weights, hls::stream<insn_T>& load_queue,
          hls::stream<bool>& g2l_dep_queue, hls::stream<bool>& l2g_dep_queue,
          bus_T inp_mem[VTA_INP_BUFF_DEPTH][INP_MAT_AXI_RATIO], bus_T wgt_mem[VTA_WGT_BUFF_DEPTH][WGT_MAT_AXI_RATIO]) {
...
#pragma HLS RESOURCE variable = y_offset_1 core = Mul_LUT latency = 4
  ...
  if (insn.memory_type == VTA_MEM_ID_INP) {
    load_pad_2d<bus_T, INP_MAT_AXI_RATIO, VTA_INP_ELEM_BYTES>(inputs, inp_mem, insn.sram_base, insn.dram_base,
                                                              insn.y_size, insn.x_size, insn.x_stride, insn.x_pad_0,
                                                              insn.x_pad_1, y_offset_0, y_offset_1);
  } else if (insn.memory_type == VTA_MEM_ID_WGT) {
    load_2d<bus_T, WGT_MAT_AXI_RATIO, VTA_WGT_ELEM_BYTES>(weights, wgt_mem, insn.sram_base, insn.dram_base, insn.y_size,
                                                          insn.x_size, insn.x_stride);
  }
...
void gemm(insn_T insn_raw, uop_T uop_mem[VTA_UOP_BUFF_DEPTH], bus_T acc_mem[VTA_ACC_BUFF_DEPTH][ACC_MAT_AXI_RATIO],
          bus_T inp_mem[VTA_INP_BUFF_DEPTH][INP_MAT_AXI_RATIO], bus_T wgt_mem[VTA_WGT_BUFF_DEPTH][WGT_MAT_AXI_RATIO],
...
// Outer Loop
EXE_OUT_LOOP:
  for (int it_out = 0; it_out < insn.iter_out; it_out++) {
  ...
  // Inner Loop
  EXE_IN_LOOP:
    for (int it_in = 0; it_in < insn.iter_in; it_in++) {
    // Iterate over micro op
    READ_GEMM_UOP:
      for (int upc = insn.uop_bgn; upc < insn.uop_end; upc++) {
#pragma HLS PIPELINE II = 1
        ...
        read_tensor<bus_T, wgt_T, wgt_idx_T, VTA_BUS_WIDTH, VTA_WGT_WIDTH, VTA_BLOCK_OUT, VTA_BLOCK_IN>(
            wgt_idx, wgt_mem, w_tensor);
        // Read in input tensor
        inp_T i_tensor[VTA_BATCH][VTA_BLOCK_IN];
        read_tensor<bus_T, inp_T, inp_idx_T, VTA_BUS_WIDTH, VTA_INP_WIDTH, VTA_BATCH, VTA_BLOCK_IN>(src_idx, inp_mem,
                                                                                                    i_tensor);
        // Read in accum tensor
        acc_T a_tensor[VTA_BATCH][VTA_BLOCK_OUT];
        read_tensor<bus_T, acc_T, acc_idx_T, VTA_BUS_WIDTH, VTA_ACC_WIDTH, VTA_BATCH, VTA_BLOCK_OUT>(dst_idx, acc_mem,
                                                                                                     a_tensor);
        // Output tensor
        ...
        write_tensor<bus_T, acc_T, acc_idx_T, VTA_BUS_WIDTH, VTA_ACC_WIDTH, VTA_BATCH, VTA_BLOCK_OUT>(dst_idx, a_tensor,
                                                                                                      acc_mem);
        // Write the results back in the output buffer
        write_tensor<bus_T, out_T, acc_idx_T, VTA_BUS_WIDTH, VTA_OUT_WIDTH, VTA_BATCH, VTA_BLOCK_OUT>(dst_idx, o_tensor,
                                                                                                      out_mem);
      }
      // Update offsets
...
void alu(insn_T insn_raw, uop_T uop_mem[VTA_UOP_BUFF_DEPTH], bus_T acc_mem[VTA_ACC_BUFF_DEPTH][ACC_MAT_AXI_RATIO],
         bus_T inp_mem[VTA_INP_BUFF_DEPTH][INP_MAT_AXI_RATIO], bus_T wgt_mem[VTA_WGT_BUFF_DEPTH][WGT_MAT_AXI_RATIO],
         bus_T out_mem[VTA_ACC_BUFF_DEPTH][OUT_MAT_AXI_RATIO]) {
#pragma HLS INLINE
...
#pragma HLS PIPELINE II = 2
        ...
        // Decode
        acc_idx_T dst_idx = uop.range(VTA_UOP_ALU_0_1, VTA_UOP_ALU_0_0) + dst_offset_in;
        acc_idx_T src_idx = uop.range(VTA_UOP_ALU_1_1, VTA_UOP_ALU_1_0) + src_offset_in;

        // Read in src tensor
        acc_T src_tensor[VTA_BATCH][VTA_BLOCK_OUT];
        read_tensor<bus_T, acc_T, acc_idx_T, VTA_BUS_WIDTH, VTA_ACC_WIDTH, VTA_BATCH, VTA_BLOCK_OUT>(src_idx, acc_mem,
                                                                                                     src_tensor);
        // Read in dst tensor
        ...
        for (int i = 0; i < VTA_BATCH; i++) {
          for (int b = 0; b < VTA_BLOCK_OUT; b++) {
            ...
            if (insn.alu_opcode == VTA_ALU_OPCODE_MIN || insn.alu_opcode == VTA_ALU_OPCODE_MAX) {
              // Compute Min/Max
              acc_T mix_val = src_0 < src_1 ? (insn.alu_opcode == VTA_ALU_OPCODE_MIN ? src_0 : src_1)
              ...
              // Compute Sum
              acc_T add_val = src_0.range(VTA_ACC_WIDTH - 1, 0) + src_1.range(VTA_ACC_WIDTH - 1, 0);
        ...
        // Write the results back into accumulator
        write_tensor<bus_T, acc_T, acc_idx_T, VTA_BUS_WIDTH, VTA_ACC_WIDTH, VTA_BATCH, VTA_BLOCK_OUT>(
            dst_idx, dst_tensor, acc_mem);
        // Write the results back in the output buffer
        write_tensor<bus_T, out_T, acc_idx_T, VTA_BUS_WIDTH, VTA_OUT_WIDTH, VTA_BATCH, VTA_BLOCK_OUT>(dst_idx, o_tensor,
                                                                                                      out_mem);
      }
...
void compute(volatile uint32_t& done, volatile uop_T* uops, volatile bus_T* biases, hls::stream<insn_T>& gemm_queue,
             hls::stream<bool>& l2g_dep_queue, hls::stream<bool>& s2g_dep_queue, hls::stream<bool>& g2l_dep_queue,
             hls::stream<bool>& g2s_dep_queue, bus_T inp_mem[VTA_INP_BUFF_DEPTH][INP_MAT_AXI_RATIO],
...
#pragma HLS DEPENDENCE variable = acc_mem inter false
  ...
  if (insn.generic.opcode == VTA_OPCODE_FINISH) {
    ...
    if (insn.mem.memory_type == VTA_MEM_ID_UOP) {
      ...
      memcpy(&uop_mem[sram_idx], (const uop_T*)&uops[dram_idx], insn.mem.x_size * sizeof(uop_T));
    } else if (insn.mem.memory_type == VTA_MEM_ID_ACC) {
      // Perform data transfer from DRAM
      load_2d<bus_T, ACC_MAT_AXI_RATIO, VTA_ACC_ELEM_BYTES>(biases, acc_mem, sram_idx, dram_idx, insn.mem.y_size,
                                                            insn.mem.x_size, insn.mem.x_stride);
    }
...
void store(volatile bus_T* outputs, hls::stream<insn_T>& store_queue, hls::stream<bool>& g2s_dep_queue,
           hls::stream<bool>& s2g_dep_queue, bus_T out_mem[VTA_ACC_BUFF_DEPTH][OUT_MAT_AXI_RATIO]) {
...
#pragma HLS PIPELINE
    // Perform data transfer
    memcpy(const_cast<bus_T*>(&outputs[dram_idx * OUT_MAT_AXI_RATIO]), (const bus_T*)&out_mem[sram_idx][0],
...
void vta(uint32_t insn_count, volatile insn_T* insns, volatile uop_T* uops, volatile bus_T* inputs,
         volatile bus_T* weights, volatile bus_T* biases, volatile bus_T* outputs) {
...
#pragma HLS INTERFACE s_axilite port = return bundle = CONTROL_BUS
  ...
  while (true) {
    ...
    while (!tmp_gemm_queue.empty() || tmp_gemm_popped == true) {
      ...
      if ((insn.pop_prev_dep && !l2g_dep_queue.empty() && insn.pop_next_dep && !s2g_dep_queue.empty()) ||
          (!insn.pop_prev_dep && insn.pop_next_dep && !s2g_dep_queue.empty()) ||
          (insn.pop_prev_dep && !l2g_dep_queue.empty() && !insn.pop_next_dep) ||
          (!insn.pop_prev_dep && !insn.pop_next_dep)) {
        // Push the instruction in the load queue
        gemm_queue.write(tmp_gemv);
        tmp_gemm_popped = false;
        compute(done, uops, biases, gemm_queue, l2g_dep_queue, s2g_dep_queue, g2l_dep_queue, g2s_dep_queue, inp_mem,
                wgt_mem, out_mem);
      } else {
    ...
    while (!tmp_store_queue.empty() || tmp_store_popped == true) {
      ...
      if ((insn.pop_prev_dep && !g2s_dep_queue.empty()) || !insn.pop_prev_dep) {
        // Push the instruction in the load queue
